...
- Guests are real auth users: `/api/guest-login` (pb_hooks/guest-auth.pb.js) mints a `guest=true` user with a synthetic `@guest.daythatworks.com` email and returns a normal auth token that the SPA stores like any login — that token IS the guest's persistent identity, so all submission rules/hooks apply unchanged. Guests cannot create events (events createRule excludes `@request.auth.guest = true`); use `<RequireAuth full>` for routes needing a real account.
